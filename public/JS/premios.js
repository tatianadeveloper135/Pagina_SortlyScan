// Funcionalidad para calcular puntuación
        function calculateScore() {
            ...
            const peopleEducated = parseInt(document.getElementById('peopleEducated').value) || 0;
            const cleanupEvents = parseInt(document.getElementById('cleanupEvents').value) || 0;
            const consecutiveDays = parseInt(document.getElementById('consecutiveDays').value) || 0;
            const reuseProjects = parseInt(document.getElementById('reuseProjects').value) || 0;
            ...
            const materialsCount = [paper, plastic, glass, metal, organic].filter(x => x > 0).length;
        ...
        document.getElementById('rankingForm').addEventListener('submit', function(e) {
            ...
            if (!fullName || !email || !city) {
                alert('⚠️ Por favor completa todos los campos obligatorios (nombre, email y ciudad).');
                return;
            }
            
            if (score === 0) {
                alert('⚠️ Debes ingresar al menos algunos datos de reciclaje para participar en el ranking.');
                return;
            }
            
            // Simular envío exitoso
            alert(`🎉 ¡Felicidades ${fullName}! Tu información ha sido enviada exitosamente.\n\n` +
        ...
        document.querySelectorAll('button').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (buttonText === 'Ver Certificado') {
                    alert('🏆 Certificado verificado y disponible para descarga. ¡Felicidades por este reconocimiento!');
                } else if (buttonText === 'Detalles') {
                    alert('📋 Mostrando detalles completos del premio. Información adicional sobre criterios y proceso de selección.');
                } else if (buttonText === 'Comenzar Ahora') {
                    alert('🚀 ¡Excelente! Te dirigiremos a la página de registro para comenzar tu viaje hacia un futuro más sostenible.');
                } else if (buttonText === 'Saber Más') {
                    alert('📚 Descubre más sobre nuestros programas, metodología y cómo puedes hacer la diferencia.');
                } else if (buttonText === 'Ver Ranking Completo →') {
                    alert('📊 Mostrando el ranking completo con todos los participantes y sus puntuaciones detalladas.');
                }
            });
        });
...
(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'96dd4c4424a5c8e4',t:'MTc1NDk3MzY2MC4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();